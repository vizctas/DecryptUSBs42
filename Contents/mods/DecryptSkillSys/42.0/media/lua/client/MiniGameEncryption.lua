-- MiniGameEncryption.lua - Encryption Cracker Minigame
-- Mastermind-style deduction game where players crack hex codes
-- Inspired by Wordle and classic code-breaking games

print("[DecryptSkillSys] Loading MiniGameEncryption.lua - Encryption Cracker system")
...
function TestEncryptionCracker(difficulty)
    ...
    if _G.MiniGame_Encryption then
        ...
        print("[DEBUG] MiniGame_Encryption function not available. Try ReloadMiniGameEncryption() first.")
    end
end
...
local WINDOW_WIDTH_PERCENT = 25
local WINDOW_HEIGHT_PERCENT = 50
local TITLE_TYPEWRITER_DELAY = 3

-- ========== GAME SETTINGS ==========
local KEY_LENGTHS = { Easy = 3, Moderate = 4, Expert = 5 }
local MAX_ATTEMPTS = { Easy = 10, Moderate = 7, Expert = 5 }
...
function MiniGameEncryptionWindow:new(x, y, width, height, player, usbType, difficulty, laptopItem, usbData)
    ...
    o.maxAttempts = MAX_ATTEMPTS[difficulty] or 10
...
function MiniGameEncryptionWindow:playSound(soundName)
...
function MiniGameEncryptionWindow:render()
    ...
    if self.gameActive then
        ...
        self:drawTextCentre(inputStr, self.width / 2, 70, 1, 1, 1, 1, UIFont.Large)
        ...
        for i, attempt in ipairs(self.attempts) do
            ...
            self:drawText(guessStr, 30, historyY, 0.8, 0.8, 0.8, 1, UIFont.Medium)
        ...
        if self.resultProcessed then
            ...
            if #self.attempts > 0 then
                ...
                if won then
                    ...
                    self:drawTextCentre("ACCESS DENIED", self.width / 2, centerY, THEME.incorrect.r, THEME.incorrect.g, THEME.incorrect.b, THEME.incorrect.a, UIFont.Large)
